 ...
 * Primitive display.
 */
public class LoveVehicle extends AbstractVehicle {
    ...
    public LoveVehicle(Point2D.Double location, double orientation) {
        this();
        this.location = location;
        this.orientation = orientation;
    }
    
    public LoveVehicle(Point2D.Double location, double orientation, boolean crossed) {
        this();
        this.location = location;
        this.orientation = orientation;
  ...
  /** Paint this vehicle.  Just a circle, a line for the direction and 2 lines for the
    ...
    public void paint(Graphics g) {
        ...
        if (mySensors.get(0).getCrossed() && mySensors.get(1).getCrossed()) {
            g.setColor(Color.MAGENTA); //hates everything
        } 
        else if (mySensors.get(0).getCrossed() && !mySensors.get(1).getCrossed()) {
            g.setColor(Color.RED); //likes heat hates light
        } 
        else if (!mySensors.get(0).getCrossed() && mySensors.get(1).getCrossed()) {
            g.setColor(Color.YELLOW); //likes light hates heat 
        } 
        else { 
            g.setColor(Color.BLUE); //likes everything
        }
    ...
    public AbstractDriveOutput step(AbstractWorld theWorld) {
        ...
        for (AbstractSensor nextSensor : mySensors) {
            if (nextSensor.getCrossed()) {
                ...
                return new LoveDriveOutput(right, left);
            }
        }
        //return new GenericDriveOutput(left, right);
        return new LoveDriveOutput(left, right);
    }
